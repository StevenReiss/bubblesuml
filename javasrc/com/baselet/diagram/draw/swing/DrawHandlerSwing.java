...
public class DrawHandlerSwing extends DrawHandler {
	...
	public DrawHandlerSwing(GridElement ge) {
		super();
		this.gridElement = ge;
	}
	...
	public void setHandler(DiagramHandler h) {
		this.handler = h;
		style = new Style();
		resetStyle();
	}
	...
	public void printHelper(StringStyle[] text, PointDouble point, AlignHorizontal align) {
		addText(new Text(text, point.x_loc * getZoom(), point.y_loc * getZoom(), align));
	}
	...
	private void drawShape(Style sty, Shape s, boolean fillShape) {
		if (fillShape) {
			// Shapes Background
			g2.setColor(Converter.convert(sty.getBackgroundColor()));
			g2.fill(s);
		}
		if (translate) {
		...
		if (sty.getLineWidth() > 0) {
			// Shapes Foreground
			ColorOwn colOwn = getOverlay().getForegroundColor() != null ? getOverlay().getForegroundColor() : sty.getForegroundColor();
			g2.setColor(Converter.convert(colOwn));
			g2.setStroke(Utils.getStroke(sty.getLineType(), (float) sty.getLineWidth()));
			g2.draw(s);
		}
	}
	...
	private void drawText(Style sty, Text t) {
		ColorOwn col = getOverlay().getForegroundColor() != null ? getOverlay().getForegroundColor() : sty.getForegroundColor();
		g2.setColor(Converter.convert(col));
		handler.getFontHandler().setFontSize(sty.getFontSize());
