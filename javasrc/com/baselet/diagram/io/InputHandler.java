...
public class InputHandler extends DefaultHandler {
	...
	public InputHandler(DiagramHandler dh) {
		this.handler = dh;
		_p = dh.getDrawPanel();
	...
	public void endElement(String uri, String localName, String qName) {
		...
		if (elementname.equals("help_text")) {
			...
			if (id != null) {
				try {
					NewGridElement nge = ElementFactorySwing.create(ElementId.valueOf(id), new Rectangle(x, y, w, h), panel_attributes, additional_attributes, handler);
					if (currentGroup != null) {
						nge.setProperty(GroupFacet.KEY, currentGroup);
					}
					_p.addElement(nge);
				} catch (Exception ex) {
	...
	private static GridElement getOldGridElementFromPath(String path)
        ...
        InvocationTargetException, NoSuchMethodException
    {
		...
		if (foundClass == null) {
			...
			return (GridElement) foundClass.getConstructor().newInstance();
		}
	}
