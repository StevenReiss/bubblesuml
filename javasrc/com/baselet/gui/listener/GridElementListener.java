...
public class GridElementListener extends UniversalListener {
	...
	public GridElementListener(DiagramHandler h) {
		super(h);
	}
	...
	private void pressedLeftButton(MouseEvent me) {
		...
		if ((me.getModifiersEx() & InputEvent.META_DOWN_MASK) != 0) {
			initializeLasso();
		}
	...
	private void initializeLasso() {
		lassoToleranceRectangle = new Rectangle(mousePressedPoint.x_loc - lassoTolerance, mousePressedPoint.y_loc - lassoTolerance, lassoTolerance * 2, lassoTolerance * 2);
		LASSO_ACTIVE = true;
	...
	private void dragEntity(boolean isShiftKeyDown, GridElement mainElement) {
		...
		int diffx = newp.x_loc - oldp.x_loc;
		int diffy = newp.y_loc - oldp.y_loc;
