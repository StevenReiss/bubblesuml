...
public class OldRelationListener extends GridElementListener {
	...
	private static class OldRemoveLinePoint extends Command {
		...
		public OldRemoveLinePoint(Relation r, int i) {
			...
			_x = p.x_loc / HandlerElementMap.getHandlerForElement(_relation).getGridSize();
			_y = p.y_loc / HandlerElementMap.getHandlerForElement(_relation).getGridSize();
		}
	...
	public OldRelationListener(DiagramHandler h) {
		super(h);
	}
	...
	public void mouseDragged(MouseEvent me) {
		...
		if (IS_DRAGGING_LINEPOINT) {
			...
			delta_x = (r.getRectangle().x + p.x_loc) % gridSize;
			delta_y = (r.getRectangle().y + p.y_loc) % gridSize;
		}

		Point newp = getNewCoordinate();
		Point oldp = getOldCoordinate();

		int diffx = newp.x_loc - oldp.x_loc - delta_x;
		int diffy = newp.y_loc - oldp.y_loc - delta_y;
