...
public class Main implements CanCloseProgram, CanOpenDiagram {
	...
	private TreeMap<String, PaletteHandler> palettes_set;
	...
	public TreeMap<String, PaletteHandler> getPalettes() {
		if (palettes_set == null) {
			palettes_set = new TreeMap<String, PaletteHandler>(Constants.DEFAULT_FIRST_COMPARATOR);
			// scan palettes
			List<File> palettes = scanForPalettes();
			for (File palette : palettes) {
				this.palettes_set.put(getFilenameWithoutExtension(palette), new PaletteHandler(palette));
			}
		}
		return palettes_set;
	}
