...
public class Line {
	...
	public Line(PointDouble s, PointDouble e) {
		super();
		this.start = s;
		this.end = e;
	}
	...
	public PointDouble getPoint(boolean s) {
		return s ? getStart() : getEnd();
	}
	...
	public double getAngleOfSlope() {
		...
		double rad = Math.acos((start.x_loc - end.x_loc) / radius);
		double radDeg = Math.toDegrees(rad);
		if (start.y_loc < end.y_loc) {
			radDeg = 360 - radDeg;
		}
		return radDeg;
	}
