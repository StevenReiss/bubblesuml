...
public class PropertiesParserState {
	...
	public PropertiesParserState(Settings s, DrawHandler d) {
		this.settings = s;
		this.drawer = d;
	}

	public void resetValues(Dimension gs, double tt, boolean ed) {
		alignment = new Alignment(settings);
		textPrintPosition = 0;
		minimumWidth = 0;
		buffer = new Buffer();
		this.gridElementSize = gs;
		elementStyle = settings.getElementStyle();
		stickingPolygonGenerator = SimpleStickingPolygonGenerator.INSTANCE;
		this.totalTextBlockHeight = tt;
		facetResponse.clear();
		drawer.setEnableDrawing(ed);
	}
	...
	public void setElementStyle(ElementStyle es) {
		this.elementStyle = es;
	}
	...
	public void setStickingPolygonGenerator(StickingPolygonGenerator g) {
		this.stickingPolygonGenerator = g;
	}
