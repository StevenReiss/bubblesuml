...
public class HierarchyFacet extends Facet {
	...
	private static final class ReferencePoint {
		...
		public ReferencePoint(PointDouble pv) {
			super();
			this.p = pv;
		}
	...
	private void updateElementSize(PropertiesParserState state, String lineWithoutTabs, PointDouble lowerRightPoint, PointDouble textPos, double widthAddon, double heightAddon) {
		state.updateMinimumSize(textPos.x_loc + widthAddon, lowerRightPoint.y_loc + heightAddon);
	}
	...
	private static void drawLinesAndUpperLeftSymbol(PointDouble lowerRightPoint, DrawHandler drawer, Cache cache, String lineWithoutTabs, int tabCount, boolean arrow) {
		if (tabCount != 0) {
			try {
				...
				PointDouble p1 = new PointDouble(lowerRightPoint.x_loc, lowerRightPoint.y_loc);
				PointDouble p2 = new PointDouble(ref.p.x_loc, lowerRightPoint.y_loc);
				PointDouble p3 = new PointDouble(ref.p.x_loc, ref.p.y_loc);
				drawer.drawLines(p1, p2, p3);
				if (!ref.hasSymbol) {
					ref.hasSymbol = true;
					if (arrow) {
						PointDouble upper = new PointDouble(ref.p.x_loc, ref.p.y_loc - ARROW_LENGTH);
						PointDouble lowerLeft = new PointDouble(ref.p.x_loc - ARROW_LENGTH / 2, ref.p.y_loc);
						PointDouble lowerRight = new PointDouble(ref.p.x_loc + ARROW_LENGTH / 2, ref.p.y_loc);
						drawer.drawLines(upper, lowerLeft, lowerRight, upper);
					}
					else {
						int dist = 2;
						double circleRadius = CIRCLE_DIAMETER / 2;
						drawer.drawCircle(ref.p.x_loc, ref.p.y_loc - circleRadius, circleRadius);
						drawer.drawLine(ref.p.x_loc, ref.p.y_loc - CIRCLE_DIAMETER + dist, ref.p.x_loc, ref.p.y_loc - dist);
						drawer.drawLine(ref.p.x_loc - circleRadius + dist, ref.p.y_loc - circleRadius, ref.p.x_loc + circleRadius - dist, ref.p.y_loc - circleRadius);
					}
				}
