...
public class LostOrFoundMessage implements LifelineOccurrence {
	 ...
	 * @param lineType
	 */
	public LostOrFoundMessage(Lifeline ll, boolean fnd, int st, String text, ArrowType at, LineType lt) {
		super();
		this.lifeline = ll;
		this.found = fnd;
		this.sendTick = st;
	...
	public Line1D draw(DrawHandler drawHandler, PointDouble topLeft, PointDouble size) {
		...
		double centerY = topLeft.y_loc + size.y_loc / 2;
		...
		if (found) {
			drawHandler.drawCircle(topLeft.x_loc + CIRCLE_RADIUS, centerY, CIRCLE_RADIUS);
			lineXStart = topLeft.x_loc + CIRCLE_RADIUS * 2;
			lineXEnd = topLeft.x_loc + size.x_loc / 2 + getCenterXOffset();
		}
		else {
			drawHandler.drawCircle(topLeft.x_loc + size.x_loc - CIRCLE_RADIUS, topLeft.y_loc + size.y_loc / 2, CIRCLE_RADIUS);
			lineXStart = topLeft.x_loc + size.x_loc / 2 + getCenterXOffset();
			lineXEnd = topLeft.x_loc + size.x_loc - CIRCLE_RADIUS * 2;
		}
		drawHandler.setBackgroundColor(oldBg);
    
		TextSplitter.drawText(drawHandler, textLines, lineXStart, topLeft.y_loc,
				lineXEnd - lineXStart, size.y_loc / 2, AlignHorizontal.CENTER, AlignVertical.BOTTOM);
	...
	public double getAdditionalYHeight(DrawHandler drawHandler, PointDouble size) {
		return TextSplitter.getSplitStringHeight(textLines,
				size.x_loc / 2 - LIFELINE_TEXT_PADDING * 2 - Math.abs(getCenterXOffset()), drawHandler
				) * 2 - size.y_loc;
	}
