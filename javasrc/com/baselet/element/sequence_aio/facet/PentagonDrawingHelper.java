...
public class PentagonDrawingHelper {
	...
	public static PointDouble draw(DrawHandler drawHandler, String[] textLines, double width, PointDouble topLeft,
		...
		TextSplitter.drawText(drawHandler, textLines, topLeft.x_loc + HEADER_TEXT_X_PADDING, topLeft.y_loc, textWidth,
		...
		double slopeStartX = topLeft.x_loc + textWidth + HEADER_TEXT_X_PADDING * 2;
		...
		drawHandler.drawLines(new PointDouble[] {
				new PointDouble(topLeft.x_loc, topLeft.y_loc + height),
				new PointDouble(slopeStartX, topLeft.y_loc + height),
				new PointDouble(slopeEndX, topLeft.y_loc + height * (1 - HEADER_PENTAGON_SLOPE_HEIGHT_PERCENTAGE)),
				new PointDouble(slopeEndX, topLeft.y_loc) });
		drawHandler.setLineType(oldLt);
		return new PointDouble(slopeEndX - topLeft.x_loc, height);
	}
}
