...
public class StateInvariant implements LifelineOccurrence {
	 ...
	 */
	public StateInvariant(String text, StateInvariantStyle sty) {
		super();
		this.style = sty;
		if (sty == StateInvariantStyle.CURLY_BRACKETS) {
			lines = ("{" + text + "}").split("\n");
		}
		else {
			lines = text.split("\n");
	...
	public Line1D draw(DrawHandler drawHandler, PointDouble topLeft, PointDouble size) {
		double height = getHeight(drawHandler, size.x_loc);
		double topY = topLeft.y_loc + (size.y_loc - height) / 2;
		...
		if (style == StateInvariantStyle.STATE) {
			...
			drawHandler.drawArc(topLeft.x_loc, topY, ROUND_PART_WIDTH * 2, height, 90, 180, true);
			drawHandler.drawArc(topLeft.x_loc + size.x_loc - ROUND_PART_WIDTH * 2, topY, ROUND_PART_WIDTH * 2, height, 270, 180, true);
			drawHandler.drawLine(topLeft.x_loc + ROUND_PART_WIDTH, topY, topLeft.x_loc + size.x_loc - ROUND_PART_WIDTH, topY);
			drawHandler.drawLine(topLeft.x_loc + ROUND_PART_WIDTH, topY + height, topLeft.x_loc + size.x_loc - ROUND_PART_WIDTH, topY + height);
			TextSplitter.drawText(drawHandler, lines, topLeft.x_loc + ROUND_PART_WIDTH * (1 - ROUND_PART_TEXT_PERCENTAGE), topY,
					size.x_loc - ROUND_PART_WIDTH * (1 - ROUND_PART_TEXT_PERCENTAGE) * 2, height, AlignHorizontal.CENTER, AlignVertical.CENTER);
		}
		else if (style == StateInvariantStyle.CURLY_BRACKETS) {
			TextSplitter.drawText(drawHandler, lines, topLeft.x_loc, topY, size.x_loc, height,
	...
	public double getAdditionalYHeight(DrawHandler drawHandler, PointDouble size) {
		return getHeight(drawHandler, size.x_loc) - size.y_loc;
	}
