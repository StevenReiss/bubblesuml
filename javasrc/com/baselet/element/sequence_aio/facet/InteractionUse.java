...
public class InteractionUse implements LifelineSpanningTickSpanningOccurrence {
	...
	public InteractionUse(int t, String text, Lifeline[] cll) {
		super();
		this.tick = t;
	...
	public void draw(DrawHandler drawHandler, DrawingInfo drawingInfo) {
		...
		drawHandler.drawRectangle(topLeft.x_loc, topLeft.y_loc, width, height);
		PointDouble pentagonSize = PentagonDrawingHelper.draw(drawHandler, HEADER_TEXT, width, topLeft);
		TextSplitter.drawText(drawHandler, textLines, topLeft.x_loc + pentagonSize.x_loc + TEXT_X_PADDING,
				topLeft.y_loc, width - (pentagonSize.x_loc + TEXT_X_PADDING) * 2, height,
				AlignHorizontal.CENTER, AlignVertical.CENTER);

		for (Lifeline ll : coveredLifelines) {
			drawingInfo.getDrawingInfo(ll).addInterruptedArea(new Line1D(topLeft.y_loc, topLeft.y_loc + height));
		}
	}
