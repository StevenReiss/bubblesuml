...
public class Message implements LifelineSpanningTickSpanningOccurrence {
	 ...
	 * @param lineType
	 */
	public Message(Lifeline f, Lifeline t, int d, int st, String text, ArrowType at, LineType lt) {
		super();
		this.from = f;
		this.to = t;
		this.duration = d;
	...
	protected void drawNormalMessage(DrawHandler drawHandler, PointDouble send, PointDouble receive,
		...
		double height = send.y_loc - drawingInfo.getVerticalStart(sendTick);
		double topLeftX;
		...
		TextSplitter.drawText(drawHandler, textLines, topLeftX, send.y_loc - height,
				Math.abs(send.x_loc - receive.x_loc) - LIFELINE_TEXT_PADDING * 2, height, hAlignment, AlignVertical.BOTTOM);
	}
	...
	protected void drawSelfMessage(DrawHandler drawHandler, PointDouble send, PointDouble receive,
		...
		double rightBorderX = Math.max(send.x_loc, receive.x_loc) + SELF_MESSAGE_LIFELINE_GAP;
		PointDouble[] msgLine = new PointDouble[] {
				send,
				new PointDouble(rightBorderX, send.y_loc),
				new PointDouble(rightBorderX, receive.y_loc),
				receive };
		...
		TextSplitter.drawText(drawHandler, textLines, rightBorderX, send.y_loc,
				lifelineXEnd - rightBorderX, receive.y_loc - send.y_loc, AlignHorizontal.LEFT, AlignVertical.CENTER);
	}
