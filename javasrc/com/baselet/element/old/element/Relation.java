...
public class Relation extends OldGridElement {
	...
	private Point getCenterOfLine() {
		...
		if (getLinePoints().size() % 2 == 1) {
			...
			ret.x_loc = (p1.x_loc + p2.x_loc) / 2;
			ret.y_loc = (p1.y_loc + p2.y_loc) / 2;
		}
		return ret;
	}
	...
	public static Point moveNextTo(Area rFixed, Rectangle rMovable, Point pStart, Point pEnd) {
		...
		int vectorX = pEnd.x_loc - pStart.x_loc;
		int vectorY = pEnd.y_loc - pStart.y_loc;

		int startx = pStart.x_loc;
		int starty = pStart.y_loc;
		int endx = pEnd.x_loc;
		int endy = pEnd.y_loc;
	...
	public boolean lineUp(Vector<Rectangle> shapes, Vector<Point> points, int hotspotx, int hotspoty) {
		...
		for (int i = points.size() - 1; i > 0; i--) {
			...
			if (p1.x_loc == p2.x_loc & p1.y_loc == p2.y_loc) {
		...
		for (int i = 0; i < shapes.size() - 1; i++) {
			...
			if (i == 0) { // The hotspot of the first element is set
				...
				r.setLocation(p.x_loc - hotspotx, p.y_loc - hotspoty);
			}
			...
			if (rMovable instanceof Arrow) {
				...
				Point diffA = new Point(-startIntersectingLine.x_loc + endIntersectingLine.x_loc, -startIntersectingLine.y_loc + endIntersectingLine.y_loc);
				Point diffB1 = new Point(diffA.y_loc, -diffA.x_loc);
				Point diffB2 = new Point(-diffB1.x_loc, -diffB1.y_loc);
				Point a1 = new Point(2 * diffA.x_loc + diffB1.x_loc, 2 * diffA.y_loc + diffB1.y_loc);
				Point a2 = new Point(2 * diffA.x_loc + diffB2.x_loc, 2 * diffA.y_loc + diffB2.y_loc);
				...
				if (arrow.getString().equals("n")) {
					...
					Point vectorA = new Point(-start.x_loc + end.x_loc, -start.y_loc + end.y_loc);
					// vector down is a vector standing 90 degrees on the line,
					// vector up is the same in the opposite direction..
					Point vectorDown = new Point(vectorA.y_loc, -vectorA.x_loc);
					Point vectorUp = new Point(-vectorDown.x_loc, -vectorDown.y_loc);
					Point newA1 = new Point(4 * vectorA.x_loc + vectorDown.x_loc, 4 * vectorA.y_loc + vectorDown.y_loc);
					Point newA2 = new Point(4 * vectorA.x_loc + vectorUp.x_loc, 4 * vectorA.y_loc + diffB2.y_loc);
			...
			rMovable.setLocation(res.x_loc, res.y_loc);
		}
	...
	public Polygon getWholeLinePolygon() {
		...
		if (s % 2 == 0 && s > 0) {
			...
			Point p1 = getLinePoints().elementAt(getLinePoints().size() / 2);
			Point p2 = getLinePoints().elementAt(getLinePoints().size() / 2 + 1);
			mid = new Point((p1.x_loc + p2.x_loc) / 2, (p1.y_loc + p2.y_loc) / 2);
		}
		else {
			return null;
		}
		...
		size = size / 2;
		p.addPoint(mid.x_loc - size, mid.y_loc - size);
		p.addPoint(mid.x_loc + size, mid.y_loc - size);
		p.addPoint(mid.x_loc + size, mid.y_loc + size);
		p.addPoint(mid.x_loc - size, mid.y_loc + size);
		return p;
	...
	public void paintEntity(Graphics g) {
		...
		for (int i = 0; i < getLinePoints().size() - 1; i++) {
			...
			if (i == Math.floor((getLinePoints().size() - 1) / 2.0)) {
				...
				// G.Mueller start
				Point pm = new Point(p1.x_loc - (p1.x_loc - p2.x_loc) / 2, p1.y_loc - (p1.y_loc - p2.y_loc) / 2);
				g2.drawLine(p1.x_loc, p1.y_loc, pm.x_loc, pm.y_loc);
				if (lineType.equals("-.")) {
					g2.setStroke(Utils.getStroke(LineType.DASHED, 1));
				}
				if (lineType.equals(".-")) {
				...
				g2.drawLine(pm.x_loc, pm.y_loc, p2.x_loc, p2.y_loc);
				...
				if (eerRelDir.indexOf("EER_SUBCLASS") >= 0) {
					...
					Point mitte = new Point(px1.x_loc - (px1.x_loc - px2.x_loc) / 2, px1.y_loc - (px1.y_loc - px2.y_loc) / 2);

					AffineTransform at = g2.getTransform();
					AffineTransform at2 = (AffineTransform) at.clone();
					int cx = mitte.x_loc;
					int cy = mitte.y_loc;
					double winkel = Utils.getAngle(px1.x_loc, px1.y_loc, px2.x_loc,
							px2.y_loc);
					at2.rotate(winkel, cx, cy);
					g2.setTransform(at2);
					g2.setColor(fgColor);
					g2.setStroke(Utils.getStroke(LineType.SOLID, 2));
					g2.drawArc(mitte.x_loc, mitte.y_loc - (int) (10 * zoom), (int) (20 * zoom), (int) (20 * zoom), 90, 180);
					g2.setStroke(Utils.getStroke(LineType.SOLID, 1));
					g2.setTransform(at);
					...
					Point r1 = new Point(mitte.x_loc, mitte.y_loc - recSize);
					Point r2 = new Point(mitte.x_loc + recSize, mitte.y_loc);
					Point r3 = new Point(mitte.x_loc, mitte.y_loc + recSize);
					Point r4 = new Point(mitte.x_loc - recSize, mitte.y_loc);

					Polygon po1 = new Polygon();
					po1.addPoint(r1.x_loc, r1.y_loc);
					po1.addPoint(r2.x_loc, r2.y_loc);
					po1.addPoint(r3.x_loc, r3.y_loc);

					Polygon po2 = new Polygon();
					po2.addPoint(r1.x_loc, r1.y_loc);
					po2.addPoint(r3.x_loc, r3.y_loc);
					po2.addPoint(r4.x_loc, r4.y_loc);

					AffineTransform at = g2.getTransform();
					AffineTransform at2 = (AffineTransform) at.clone();
					int cx = mitte.x_loc;
					int cy = mitte.y_loc;
					double winkel = Utils.getAngle(px1.x_loc, px1.y_loc, px2.x_loc, px2.y_loc);
					at2.rotate(winkel, cx, cy);
					g2.setTransform(at2);
				...
				// A.Mueller start
					...
					Point outerArc = new Point(mitte.x_loc - (int) (15 * zoom), mitte.y_loc - (int) (15 * zoom));
					Point innerCircle = new Point();

					g2.setColor(Color.white);
					g2.fillOval(outerArc.x_loc, outerArc.y_loc, (int) (30 * zoom), (int) (30 * zoom));
					g2.setColor(fgColor);
					g2.setStroke(Utils.getStroke(LineType.SOLID, 1));

					if (clientServer.equals("provideRequire")) {
						g2.drawArc(outerArc.x_loc, outerArc.y_loc, (int) (30 * zoom), (int) (30 * zoom), 90, 180);
						innerCircle = new Point(mitte.x_loc - (int) (5 * zoom), mitte.y_loc - (int) (10 * zoom));
					}
					else if (clientServer.equals("requireProvide")) {
						g2.drawArc(outerArc.x_loc, outerArc.y_loc, (int) (30 * zoom), (int) (30 * zoom), 90, -180);

						innerCircle = new Point(mitte.x_loc - (int) (15 * zoom), mitte.y_loc - (int) (10 * zoom));
					}

					g2.drawOval(innerCircle.x_loc, innerCircle.y_loc, (int) (20 * zoom), (int) (20 * zoom));
					g2.setTransform(at);
				}
				// A.Mueller end
				// G.Mueller start
					...
					Point circle = new Point(mitte.x_loc - (int) (15 * zoom), mitte.y_loc - (int) (15 * zoom));
					if (middleArrow.equals("delegation")) {
						g2.setColor(Color.white);
						g2.fillOval(circle.x_loc + (int) (5 * zoom), circle.y_loc + (int) (5 * zoom), (int) (20 * zoom), (int) (20 * zoom));
						g2.setColor(fgColor);
						g2.setStroke(Utils.getStroke(LineType.SOLID, 1));
						g2.drawOval(circle.x_loc + (int) (5 * zoom), circle.y_loc + (int) (5 * zoom), (int) (20 * zoom), (int) (20 * zoom));
					}
					if (middleArrow.startsWith("delegationArrow")) {
						g2.setStroke(Utils.getStroke(LineType.SOLID, 1));
						if (middleArrow.equals("delegationArrowRight")) {
							g2.drawLine(circle.x_loc + (int) (5 * zoom), circle.y_loc + (int) (15 * zoom), circle.x_loc - (int) (5 * zoom), circle.y_loc + (int) (9 * zoom));
							g2.drawLine(circle.x_loc + (int) (5 * zoom), circle.y_loc + (int) (15 * zoom), circle.x_loc - (int) (5 * zoom), circle.y_loc + (int) (20 * zoom));
						}
						if (middleArrow.equals("delegationArrowLeft")) {
							g2.drawLine(circle.x_loc + (int) (25 * zoom), circle.y_loc + (int) (15 * zoom), circle.x_loc + (int) (35 * zoom), circle.y_loc + (int) (9 * zoom));
							g2.drawLine(circle.x_loc + (int) (25 * zoom), circle.y_loc + (int) (15 * zoom), circle.x_loc + (int) (35 * zoom), circle.y_loc + (int) (20 * zoom));
						}
					...
					if (middleArrow.equals("delegationStart")) {
						g2.setColor(Color.white);
						g2.fillArc(circle.x_loc, circle.y_loc, (int) (30 * zoom), (int) (30 * zoom), 90, 180);
						g2.setColor(fgColor);
						g2.setStroke(Utils.getStroke(LineType.SOLID, 1));
						g2.drawArc(circle.x_loc, circle.y_loc, (int) (30 * zoom), (int) (30 * zoom), 90, 180);
					}
				...
				Point p1 = getLinePoints().elementAt(i);
				Point p2 = getLinePoints().elementAt(i + 1);
				g2.drawLine(p1.x_loc, p1.y_loc, p2.x_loc, p2.y_loc);
			}
		}
		...
		if (HandlerElementMap.getHandlerForElement(this).getDrawPanel().getSelector().isSelected(this)) {
			for (int i = 0; i < getLinePoints().size(); i++) {
				...
				g2.drawOval(p.x_loc - start, p.y_loc - start, width, width);
			}
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (r instanceof Qualifier) {
				...
				if (arrow.getString().equals("crowsFoot")) {
					...
					g2.drawLine(arrow.getX(), arrow.getY() + arrow.getArrowEndA().y_loc,
							arrow.getX() + arrow.getArrowEndA().x_loc, arrow.getY());
					g2.drawLine(arrow.getX(), arrow.getY() + arrow.getArrowEndB().y_loc,
							arrow.getX() + arrow.getArrowEndB().x_loc, arrow.getY());
					...
					g2.drawLine(arrow.getX(), arrow.getY(),
							arrow.getX() + arrow.getArrowEndA().x_loc,
							arrow.getY() + arrow.getArrowEndA().y_loc);
					g2.drawLine(arrow.getX(), arrow.getY(),
							arrow.getX() + arrow.getArrowEndB().x_loc,
							arrow.getY() + arrow.getArrowEndB().y_loc);
					// A.Mueller start
				}
				// A.Mueller end
				...
				if (arrow.getString().equals("<<<")) { // LME
					...
					int[] ay = new int[3];
					ax[0] = arrow.getX();
					ax[1] = arrow.getX() + arrow.getArrowEndA().x_loc;
					ax[2] = arrow.getX() + arrow.getArrowEndB().x_loc;
					ay[0] = arrow.getY();
					ay[1] = arrow.getY() + arrow.getArrowEndA().y_loc;
					ay[2] = arrow.getY() + arrow.getArrowEndB().y_loc;
					...
					ax[1] = arrow.getX() + arrow.getArrowEndA().x_loc;
					ay[1] = arrow.getY() + arrow.getArrowEndA().y_loc;
					ax[3] = arrow.getX() + arrow.getArrowEndB().x_loc;
					ay[3] = arrow.getY() + arrow.getArrowEndB().y_loc;
				...
				// A.Mueller Begin
					...
					g2.drawLine(arrow.getX() + arrow.getArrowEndA().x_loc,
							arrow.getY() + arrow.getArrowEndA().y_loc,
							arrow.getX() + a2.x_loc,
							arrow.getY() + a2.y_loc);
					g2.drawLine(arrow.getX() + arrow.getArrowEndB().x_loc,
							arrow.getY() + arrow.getArrowEndB().y_loc,
							arrow.getX() + a1.x_loc,
							arrow.getY() + a1.y_loc);
					...
					int width = arrow.getCrossEndB().x_loc - arrow.getCrossEndA().x_loc;
					int height = arrow.getCrossEndB().y_loc - arrow.getCrossEndA().y_loc;
					g2.drawOval(arrow.getX() + arrow.getCrossEndA().x_loc, arrow.getY() + arrow.getCrossEndA().y_loc, width, height);

				}
				else if (arrow.getString().equals("provide")) {
					int width = arrow.getCrossEndB().x_loc - arrow.getCrossEndA().x_loc;
					int height = arrow.getCrossEndB().y_loc - arrow.getCrossEndA().y_loc;
					g2.drawArc(arrow.getX() + arrow.getCrossEndA().x_loc, arrow.getY() + arrow.getCrossEndA().y_loc, width, height, arrow.getArcStart(), arrow.getArcEnd());
					// A.Mueller End
					// G.Mueller Start
				}
					...
					g2.setTransform(at2);
					g2.setColor(bgColor);
					g2.fillOval(px1.x_loc, px1.y_loc - (int) (10 * zoom), (int) (20 * zoom), (int) (20 * zoom));
					g2.setColor(fgColor);
					g2.drawOval(px1.x_loc, px1.y_loc - (int) (10 * zoom), (int) (20 * zoom), (int) (20 * zoom));
					g2.drawLine(px1.x_loc + (int) (10 * zoom), px1.y_loc - (int) (5 * zoom), px1.x_loc + (int) (10 * zoom), px1.y_loc + (int) (5 * zoom));
					g2.drawLine(px1.x_loc + (int) (15 * zoom), px1.y_loc, px1.x_loc + (int) (5 * zoom), px1.y_loc);
					g2.setTransform(at);
					...
					if (beginArrow.startsWith("fill_poly_start")) {
						...
						AffineTransform at2 = (AffineTransform) at.clone();
						double winkel = Utils.getAngle(px1.x_loc, px1.y_loc, px2.x_loc, px2.y_loc);
						at2.rotate(winkel, px1.x_loc, px1.y_loc);
						g2.setTransform(at2);
						int[] x_cord = { px1.x_loc, px1.x_loc + (int) (13 * zoom), px1.x_loc + (int) (13 * zoom) };
						int[] y_cord = { px1.y_loc, px1.y_loc - (int) (7 * zoom), px1.y_loc + (int) (7 * zoom) };
					...
					if (endArrow.startsWith("fill_poly_end")) {
						...
						AffineTransform at = g2.getTransform();
						AffineTransform at2 = (AffineTransform) at.clone();
						double winkel = Utils.getAngle(px2.x_loc, px2.y_loc, px1.x_loc, px1.y_loc);
						at2.rotate(winkel, px1.x_loc, px1.y_loc);
						g2.setTransform(at2);
						int[] x_cord = { px1.x_loc, px1.x_loc - (int) (13 * zoom), px1.x_loc - (int) (13 * zoom) };
						int[] y_cord = { px1.y_loc, px1.y_loc - (int) (7 * zoom), px1.y_loc + (int) (7 * zoom) };
					...
					if (beginArrow.startsWith("compStart")) {
						...
						px2 = getLinePoints().elementAt(1);
						g2.setColor(bgColor);
						g2.fillRect(px1.x_loc - s / 2, px1.y_loc - s / 2, s, s);
						g2.setColor(fgColor);
						g2.drawRect(px1.x_loc - s / 2, px1.y_loc - s / 2, s, s);
						if (csdStartText.equals(">")) {
							int[] tmpX = { px1.x_loc - arrowOneSize, px1.x_loc + arrowOneSize, px1.x_loc - arrowOneSize };
							int[] tmpY = { px1.y_loc - arrowOneSize, px1.y_loc, px1.y_loc + arrowOneSize };
							...
							int[] tmpX = { px1.x_loc - arrowOneSize, px1.x_loc, px1.x_loc + arrowOneSize };
							int[] tmpY = { px1.y_loc - arrowOneSize, px1.y_loc + arrowOneSize, px1.y_loc - arrowOneSize };
							g2.fillPolygon(tmpX, tmpY, 3);
						}
						else if (csdStartText.equals("^")) {
							int[] tmpX = { px1.x_loc - arrowOneSize, px1.x_loc, px1.x_loc + arrowOneSize };
							...
							g2.drawLine(px1.x_loc - arrowTwoSize, px1.y_loc - arrowTwoSeparator, px1.x_loc + arrowTwoSize, px1.y_loc - arrowTwoSeparator);
							g2.drawLine(px1.x_loc + arrowTwoSize, px1.y_loc - arrowTwoSeparator, px1.x_loc + 1, px1.y_loc - arrowTwoSize);
							g2.drawLine(px1.x_loc - arrowTwoSize, px1.y_loc + arrowTwoSeparator, px1.x_loc + arrowTwoSize, px1.y_loc + arrowTwoSeparator);
							g2.drawLine(px1.x_loc - arrowTwoSize, px1.y_loc + arrowTwoSeparator, px1.x_loc - 1, px1.y_loc + arrowTwoSize);
						}
						else {
							if (!csdStartText.equals("")) {
								HandlerElementMap.getHandlerForElement(this).getFontHandler().writeText(g2, csdStartText, px1.x_loc, px1.y_loc + (int) (6 * zoom), AlignHorizontal.CENTER);
							}
						}
						...
						if (beginArrow.equals("compStartdel")) {
							...
							at2.rotate(winkel, cx, cy);
							g2.setTransform(at2);
							g2.drawLine((int) (px1.x_loc + s / 2 + 2 * zoom), px1.y_loc, px1.x_loc + s / 2 + arrowThreeLength, px1.y_loc - arrowThreeSize);
							g2.drawLine((int) (px1.x_loc + s / 2 + 2 * zoom), px1.y_loc, px1.x_loc + s / 2 + arrowThreeLength, px1.y_loc + arrowThreeSize);
							g2.setTransform(at);
						}
					...
					if (endArrow.startsWith("compEnd")) {
						...
						if (csdEndText.equals(">")) {
							...
							if (!csdEndText.equals("")) {
								HandlerElementMap.getHandlerForElement(this).getFontHandler().writeText(g2, csdEndText, px1.x_loc, px1.y_loc + (int) (6 * zoom), AlignHorizontal.CENTER);
							}
						}
		...
		if (getStrings() != null) {
			if (getStrings().size() > 0) {
				...
				int yPos = start.y_loc - (int) HandlerElementMap.getHandlerForElement(this).getFontHandler().getDistanceBetweenTexts(); // B. Buckl
				// added
				// -this.getHandler().getDistTextToText()
				int xPos = start.x_loc;
		...
		for (int i = 0; i < getLinePoints().size(); i++) {
			Point p = getLinePoints().elementAt(i);
			minx = Math.min(minx, p.x_loc);
			miny = Math.min(miny, p.y_loc);
			maxx = Math.max(maxx, p.x_loc);
			maxy = Math.max(maxy, p.y_loc);

			// Subtract or add the SELECTCIRCLESIZE to avoid cutting the circles at the end of the relation
			minx = (int) Math.min(minx, p.x_loc - SELECTCIRCLESIZE * zoom);
			miny = (int) Math.min(miny, p.y_loc - SELECTCIRCLESIZE * zoom);
			maxx = (int) Math.max(maxx, p.x_loc + SELECTCIRCLESIZE * zoom);
			maxy = (int) Math.max(maxy, p.y_loc + SELECTCIRCLESIZE * zoom);
		}
	...
	public PointDouble getAbsoluteCoorStart() {
		PointDouble ret = new PointDouble(getRectangle().x + getStartPoint().x_loc, getRectangle().y + getStartPoint().y_loc);
		return ret;
	}

	public PointDouble getAbsoluteCoorEnd() {
		PointDouble ret = new PointDouble(getRectangle().x + getEndPoint().x_loc, getRectangle().y + getEndPoint().y_loc);
		return ret;
	}
	...
	public boolean allPointsOnSamePos() {
		...
		for (Point p : getLinePoints()) {
			if (first == null) {
				...
				if (first.x_loc != p.x_loc || first.y_loc != p.y_loc) {
					return false;
				}
			}
		}
