...
public abstract class CustomElement extends OldGridElement {
	...
	private static class Text {
		...
		private Text(String t, int xv, int yv, AlignHorizontal a) {
			this.text = t;
			this.x = xv;
			this.y = yv;
			this.align = a;
		...
		private Text(String t, int xv, int yv, AlignHorizontal a, Integer sz) {
			this.text = t;
			this.x = xv;
			this.y = yv;
			this.align = a;
			this.fixedSize = sz; // some texts should not be zoomed
		}
	}
	...
	public final void setWordWrap(boolean ww) {
		this.wordWrap = ww;
	}
	...
	protected final void drawArcOpen(float x, float y, float w, float h, float start, float extent) {
		shapes.add(new StyleShape(new Arc2D.Float(x * zoom, y * zoom, w * zoom, h * zoom, start, extent, Arc2D.OPEN), tmpLineType, tmpLineThickness, tmpFgColor, tmpBgColor, tmpAlpha));
	}

	@CustomFunction(param_defaults = "x, y, width, height, start, extent")
	protected final void drawArcChord(float x, float y, float w, float h, float start, float extent) {
		shapes.add(new StyleShape(new Arc2D.Float(x * zoom, y * zoom, w * zoom, h * zoom, start, extent, Arc2D.CHORD), tmpLineType, tmpLineThickness, tmpFgColor, tmpBgColor, tmpAlpha));
	}

	@CustomFunction(param_defaults = "x, y, width, height, start, extent")
	protected final void drawArcPie(float x, float y, float w, float h, float start, float extent) {
		shapes.add(new StyleShape(new Arc2D.Float(x * zoom, y * zoom, w * zoom, h * zoom, start, extent, Arc2D.PIE), tmpLineType, tmpLineThickness, tmpFgColor, tmpBgColor, tmpAlpha));
	}
	...
	protected final void drawRectangle(int x, int y, int w, int h) {
		shapes.add(new StyleShape(new Rectangle((int) (x * zoom), (int) (y * zoom), (int) (w * zoom), (int) (h * zoom)), tmpLineType, tmpLineThickness, tmpFgColor, tmpBgColor, tmpAlpha));
	}

	@CustomFunction(param_defaults = "x, y, width, height, arcw, arch")
	protected final void drawRectangleRound(int x, int y, int w, int h, float arcw, float arch) {
		shapes.add(new StyleShape(new RoundRectangle2D.Float((int) (x * zoom), (int) (y * zoom), (int) (w * zoom), (int) (h * zoom), arcw * zoom, arch * zoom), tmpLineType, tmpLineThickness, tmpFgColor, tmpBgColor, tmpAlpha));
	}
