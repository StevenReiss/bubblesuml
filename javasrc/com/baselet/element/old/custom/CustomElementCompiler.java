...
public class CustomElementCompiler {
	...
	private CustomElement compile(String code) {
		...
		try {
			...
			String sf = "\"" + this.sourcefile.getAbsolutePath() + "\"";
			...
			if (compilationSuccessful) {
				...
				if (c != null) {
					entity = (CustomElement) c.getConstructor().newInstance();
				}
			}
			else {
	...
	private String createClasspath() {
           // TODO: handle bubbles class path
	...
	private String parseCodeFromTemplate(String t) {
		Matcher m = template_pattern.matcher(t);
		if (m.matches()) {
			return m.group(3);
		}
		else {
	...
	public GridElement genEntityFromTemplate(String templatename, ErrorHandler errorhandler) {
		String t = loadJavaSource(new File(Path.customElements() + templatename + ".java"));
		if (!"".equals(t)) {
			return this.genEntity(parseCodeFromTemplate(t), errorhandler);
		}
