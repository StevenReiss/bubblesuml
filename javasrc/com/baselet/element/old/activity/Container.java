...
public class Container extends Element {
	...
	public Container(AtomicBoolean ai, DiagramHandler handler, Graphics2D g, Container p, ArrayList<Row> rws, int row) {
	...
	private Row getFirstRow() {
		int initrow = this.init_row;
		if (start != null) {
			initrow--;
		}
		while (rows.size() <= initrow) {
			rows.add(new Row());
		}
		return rows.get(initrow);
	}

	private Row getLastRow() {
		int maxrow = this.max_row;
		if (stop != null) {
			maxrow--;
		}
