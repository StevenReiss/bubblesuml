...
public class PlotDrawHandler {
	...
	private final void drawLineOrPoints(boolean xIsDescription, Double[][] vals, int sourceAxisPos, int valueAxisPos, Double valueSegment, int descSegment, List<String> clrs, boolean line) {
		int cIndex = 0;
		for (int valueIndex = 0; valueIndex < vals.length; valueIndex++) {
			Double[] vArray = vals[valueIndex];
			int actualValPos;
			int lineIterator = valueAxisPos + descSegment / 2;
			...
			base.setForegroundColor(ColorOwn.forStringOrNull(clrs.get(cIndex), Transparency.FOREGROUND));
			base.setBackgroundColor(ColorOwn.forStringOrNull(clrs.get(cIndex), Transparency.FOREGROUND));

			if (line) {
				for (int i = 0; i < points.size() - 1; i++) {
					Point point1 = points.get(i);
					Point point2 = points.get(i + 1);
					base.drawLine(point1.x_loc, point1.y_loc, point2.x_loc, point2.y_loc);
				}
			}
			else {
				for (int i = 0; i < points.size(); i++) {
					Point point = points.get(i);
					base.drawCircle(point.x_loc, point.y_loc, 2);
				}
			}
			// print titleCol
			base.setForegroundColor(ColorOwn.forStringOrNull(clrs.get(cIndex), Transparency.FOREGROUND).darken(75));
			base.print(title[valueIndex], points.get(points.size() - 1).x_loc, points.get(points.size() - 1).y_loc, AlignHorizontal.CENTER);
	...
	private final void drawBars(boolean xIsDescription, Double[][] vals, int sourceAxisPos, int valueAxisPos, Double valueSegment, int descSegment, List<String> clrs) {
		int barLength;
		int valueRowAmount = vals.length;
		for (int vIndex = 0; vIndex < valueRowAmount; vIndex++) {
			int cIndex = 0;
			int subBarIterator = valueAxisPos;
			for (Double v : vals[vIndex]) {
				if (cIndex >= clrs.size()) {
					cIndex = 0; // Restart with first color if all colors in the array has been used
				}
				base.setForegroundColor(ColorOwn.TRANSPARENT);
				base.setBackgroundColorAndKeepTransparency(clrs.get(cIndex));
	...
	private final void drawPieArcs(Double[] vals, String[] d, Point ulCorner, int diameter, Double valueSum, List<String> cols) {
		int cIndex = 0;
		...
		for (int i = 0; i < vals.length; i++) {
			...
			base.setForegroundColor(ColorOwn.TRANSPARENT);
			base.setBackgroundColorAndKeepTransparency(cols.get(cIndex));
    
			arcAngle = i < vals.length - 1 ? Math.round(360.0 / valueSum * Math.abs(vals[i])) : 360 - startAngle;
			...
			base.drawArc(ulCorner.x_loc + width / 2.0 - diameter / 2.0, ulCorner.y_loc + height / 2.0 - diameter / 2.0, diameter, diameter, startAngle.floatValue(), arcAngle.floatValue(), false);
			base.setForegroundColor(currentFg);
    
			double radians = (360 - startAngle + (360 - arcAngle / 2)) * Math.PI / 180.0;
			int value_x = (int) (diameter / 2.0 * Math.cos(radians) + ulCorner.x_loc + diameter / 2.0 + width / 2.0 - diameter / 2.0);
			int value_y = (int) (diameter / 2.0 * Math.sin(radians) + ulCorner.y_loc + diameter / 2.0 + height / 2.0 - diameter / 2.0);
    
			base.setForegroundColor(ColorOwn.forStringOrNull(cols.get(cIndex), Transparency.FOREGROUND).darken(75));
			base.print(d[i], value_x, value_y, AlignHorizontal.CENTER);
	...
	public void setValues(String[] d, String[] t, Double[][] vals, List<String> c) {
		this.desc = SharedUtils.cloneArray(d);
		this.title = SharedUtils.cloneArray(t);
		this.colors = new ArrayList<String>(c);
		this.values = SharedUtils.cloneArray(vals);

		valuesSorted = new TreeSet<Double>();
		for (Double[] vArray : vals) {
			for (Double v : vArray) {
				valuesSorted.add(v);
			}
		}
	...
	public void setMinValue(Double mv) throws IOException {
	...
	public void setMaxValue(Double mv) throws IOException {
