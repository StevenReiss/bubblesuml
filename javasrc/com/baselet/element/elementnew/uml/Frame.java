...
public class Frame extends NewGridElement {
	...
	protected void drawCommonContent(PropertiesParserState s) {
		DrawHandler drawer = s.getDrawer();
		drawer.drawRectangle(0, 0, getRealSize().getWidth(), getRealSize().getHeight());
    
		TextBeforeFirstSeparatorCollectorFacetResponse packageTitleResponse = s.getFacetResponse(TextBeforeFirstSeparatorCollectorFacet.class, null);
		if (packageTitleResponse != null) {
			double top = s.getBuffer().getTop();
			...
			double lowerLeftSpace = s.getXLimits(heightOfTitle).getLeft();
			...
			s.getBuffer().setTopMin(heightOfTitle);
